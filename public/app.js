...
// 2026-09-14에 gemini-3.7-flash로 다시 재고 1,200/1,500에서 올렸다. 그전 값은 3.6
// Flash 시절 것이라 지금 모델에는 지나치게 빡빡했다 — 3,137자도 문장 누락 없이
// 그대로 나온다. 품질이 아니라 **시간**만이 상한을 정한다.
//
//   지문      문장    1차 호출   총 시간   모델 호출   결과
//   1,556자   14      26.8초     33.1초    2회        누락 0
//   2,097자   17      32.5초     51.3초    4회        누락 0
//   3,137자   24      43.8초     81.1초    5회        누락 0
//
// 1,000자당 26초꼴로 거의 정비례한다. 늘어나는 몫은 1차 호출(26.8→43.8초)이 아니라
// 보정 왕복 횟수(2→5회)다 — 길수록 모델이 놓치는 자리가 늘어난다.
// 배포 프록시가 약 100초에서 끊으므로(server.py의 '프록시 제한' 주석 참고) 그 선이
// 곧 상한이고, 여기 숫자는 위 실측이 로컬 PC 기준이라 Render가 더 느린 것을 감안해
// 잡았다. 2,500자 ≒ 65초로 35초쯤 여유가 남는다.
const PASSAGE_WARN = 2000;
const PASSAGE_DANGER = 2500;
...
const MCQ_TYPES = [
  ...
  { id: "목적", def: false }, { id: "심경", def: false }, { id: "지칭 추론", def: false },
...
const SAQ_TYPES = [
  ...
  // label: 칸에 보일 이름만 줄인 것 — 실제 유형 이름은 그대로다. 이 둘도 유독 길어
  // 칩 안에서 낱말이 끊기며 줄바꿈됐다(예: "문맥의" / "미 매칭형"). id를 줄이면
  // 서버·저장본과 어긋나므로 표시만 줄인다 — "요약문 완성"과는 뜻이 겹치지 않게
  // "구성형"(요약문을 통째로 새로 짓는다)으로, 다의어 쪽은 핵심 낱말만 남긴다.
  { id: "다의어 문맥의미 매칭형", label: "다의어 매칭형", def: false },
  ...
  { id: "요약문 완전구성형", label: "요약문 구성형", def: false },
];
...
const TYPE_MAX_REASON = {
  ...
  심경: "이야기 하나의 정서적 흐름은 보통 하나뿐이라",
  "지칭 추론": "대명사가 헷갈릴 만큼 대상이 겹치는 자리 수 때문에",
  "어법 분석": "지문에 실제로 있는 문법 포인트 수 때문에",
  "옳은 문장 찾기": "지문에 실제로 있는 문법 포인트 수 때문에",
  "영영풀이 오류 찾기": "영영풀이 하나로 뜻이 딱 정해지는 낱말 수 때문에",
...
/* 객관식 '지문변형형' — 정찰 가격이 다른 유형들(server.py의 _quiz_type_base_price).
   ...
   (숫자를 못박아 두지 않는다 — 예전에 "8개"라고 적어 뒀다가 유형이 늘면서 12개가
   됐는데도 안 고쳐져 몇 년째 틀린 채로 남아 있었다).

   ⚠️ 예전에 함축의미가 빠져 있었다. 두 단가가 250원으로 같던 동안에는 드러나지 않다가,
   값을 갈라 놓는 순간 화면이 보여 준 예상 금액보다 실제로 더 많이 빠져나가는 자리다
   (실제 청구는 언제나 서버가 다시 계산한다). 유형을 늘릴 때 이 목록을 함께 보라.

   지문을 '손대는' 기준은 빈칸·네모만이 아니다 — 무관한 문장은 없던 문장을 지어 끼워
   넣고, 요약문은 요약 문장을 새로 쓰고, 함축의미·지칭 추론·어법 분석·옳은 문장 찾기·
   영영풀이 오류 찾기는 지문에 밑줄(과 정의)을 그어 넣고, 연결어 2빈칸 추론은 담화표지
   두 자리를 빈칸으로 뚫는다.
   목적·심경은 반대로 지문을 그대로 읽고 판단만 하므로 여기 들지 않는다(지문유지형). */
...
/* 유형을 호출 묶음으로 나눈다. 규칙 세 가지:
   ...
   ③ 그 안에서 문항 수 합계가 상한을 넘지 않게 묶는다. 단, 유형 하나가 이미 상한보다
      크면(예: 빈칸·어휘 6문항) ②가 우선이라 그 유형만으로 한 묶음을 만든다.
...
function setupQuizTab({ prefix, types, footer }) {
  ...
  /* ── 만들어 둔 문제의 순서만 다시 섞기 ──
     ...
     저장함에서 불러온 자료에도 그대로 쓸 수 있고, 몇 번을 눌러도 그때마다 섞인다. */
  function reshuffle() {
    /* 변형 세트끼리는 섞지 않는다 — 같은 지문의 원문판·변형판이 한 벌에 뒤섞이면
       거의 같은 지문을 두 번 풀게 되고, 애초에 A형/B형처럼 여러 벌을 뽑으려고
       고르는 기능이기 때문이다. 그 밖에는 지문 구분 없이 통째로 섞는다.
       변형 세트 표식이 없던 시절에 저장한 자료는 담겨 있던 정보로 대신 채운다. */
    const groups = new Map();
    (lastEntries || []).forEach((e) => {
      const set = e.set || {};
      (set.questions || []).forEach((q) => {
        const key = q.__label != null ? q.__label : e.label || "";
        if (!groups.has(key)) {
          groups.set(key, { rank: Infinity, questions: [], seen: new Set(), variations: [] });
        }
    ...
    [...groups.entries()]
      ...
      .forEach(([label, g]) => {
        ...
        // 이름표(변형 세트 이름)는 묶음이 둘 이상일 때만 붙는다 — passageBanner가 total로 판단한다
        entries.push({ job: { name: label || "", named: false }, label: "", set, total: groups.size });
      });
    renderQuizEntries(entries);
  }

  // 섞기 버튼은 결과가 있을 때만 보인다
  function syncShuffleBtns() {
    if (!shuffleBtn) return;
    shuffleBtn.style.display = (lastEntries || []).length ? "inline-flex" : "none";
    syncFloatPrint();
  }
...
// 내신 서술형 시험지의 <조건> 상자 — 조건 영작·문장 전환이 함께 쓴다.
// 조건이 없으면 상자째 빼서 빈 테두리만 남지 않게 한다.
function quizConditionsHtml(q) {
  const list = (q.conditions || []).filter((c) => String(c || "").trim());
  if (!list.length) return "";
  // 번호는 ol의 기본 표시(1.)가 아니라 시험지에서 쓰는 ①②③로 직접 붙인다
  const rows = list
    .map(
      (c, i) =>
        `<li><span class="qz-num">${CIRCLED[i] || i + 1}</span><span>${esc(c)}</span></li>`
    )
    .join("");
  return `<div class="qz-conditions">
    <span class="qz-conditions-tag">&lt;조건&gt;</span>
    <ul>${rows}</ul></div>`;
}
...
function quizBodyHtml(q) {
  ...
  /* 조건 영작 — 지문 만드는 방법은 write와 같다(문장 하나만 우리말로 바뀌어 밑줄+굵게).
     다른 점은 학생이 하는 일이다. <보기> 낱말이 '원형'으로 주어져 형태를 스스로 정해야
     하고, <조건>의 문법·낱말 수를 함께 맞춰야 한다. 그래서 배열처럼 정답 문장을 그대로
     흩어 놓지 않고, 서버가 준 wordBank를 섞어 보여 준다. */
  if (fmt === "compose") {
  ...
  /* 문장 전환 — 지문은 손대지 않고 바꿔 쓸 문장 하나만 밑줄+굵게. 무엇으로 바꾸는지는
     문항마다 달라서 <조건>이 들고 있다(발문은 "…<조건>에 맞게 바꿔 쓰시오."로 고정). */
  if (fmt === "convert") {
    return `
      <div class="qz-passage">${safeHTML(q.passageHtml)}</div>
      ${quizConditionsHtml(q)}
...
function buildQuizHtml(d, job, total, kind, label, sheetHead, showExp = true, startNo = 0) {
  ...
  /* 지문 이름표는 첫 문항과 **한 상자(.qz-lead)에 묶어** 내보낸다.
     쪽을 떼지 않고 이어 붙이게 바꾼 뒤로 이름표만 앞 쪽에 남는 일이 있었다 —
     '21번'만 덩그러니 찍힌 쪽이 나오고 문제는 다음 쪽에서 시작했다.
     두 번을 헛짚었으니 까닭을 적어 둔다.
       ① break-after:avoid로 막으려 했다 → 크로미움이 break-before/after의 avoid를
          구현하지 않아 무시한다. 듣는 것은 break-inside의 avoid뿐이다.
       ② 이름표를 2단 조판 래퍼(.qz-cards) 안으로 옮겼다 → 래퍼가 쪽 경계에서 갈라질
          때 이름표만 앞 조각에 남아 그대로였다. 같은 래퍼에 있다고 붙어 다니지 않는다.
     첫 문항 카드는 break-inside:avoid라 통째로 움직인다. 이름표를 그 카드와 한 상자에
     넣고 상자에도 같은 규칙을 주면, 카드가 다음 쪽으로 밀릴 때 이름표도 딸려 간다.
     화면에서는 .qz-lead에 아무 규칙이 없어 보이는 모양이 달라지지 않는다. */
  if (banner && cards.length) {
    parts.push(`<div class="qz-lead">${banner}${cards[0]}</div>`);
    parts.push(cards.slice(1).join(""));
  } else {
...
const HOWTO = {
  ...
  brief: {
    ...
    steps: [
      ...
      "(선택) <b>표지 제목</b>을 적으면 첫 장에 표지가 붙습니다.",
      "(선택) <b>요약 이미지</b>를 고르면 지문을 한 장으로 요약한 그림이 맨 뒤에 붙습니다 — 한국어＋영어 · 한글요약 · 영어요약.",
      "<b>[요약분석 만들기]</b>를 누릅니다.",
      "<b>[📄 쪽 구성]</b>을 누르면 인쇄했을 때 쪽이 어디서 넘어가는지 보이고, 앞 쪽에 몇 mm가 비는지도 알려 줍니다. 카드 위 단추로 경계를 옮기거나 <b>[⤴ 전체 올리기]</b>로 한 번에 채우세요.",
      "<b>[🖨️ 인쇄 / PDF 변환]</b> · <b>[💾 사이트 저장]</b> — 저장한 것은 <b>[📂 불러오기]</b>로 되불러옵니다.",
    ],
  ...
  mcq: {
    ...
    steps: [
      ...
      "<b>[🖨️ 인쇄 / PDF 변환]</b> · <b>[🔑 답지만]</b>(정답 및 해설만 인쇄) · <b>[💾 사이트 저장]</b>. <b>답지에 해설</b>을 끄면 답지에 정답만 나옵니다.",
    ],
    tip: "한 번에 200문항이 넘으면 나눠서 만들지 물어봅니다 — 한 번에 너무 많이 만들면 중간에 실패할 확률이 커지기 때문입니다.",
    sampleHead: "객관식은 이렇게 나옵니다 — 한 지문으로 23가지 유형을 한 문항씩 만든 시험지 중 세 쪽입니다",
  },
  saq: {
    ...
    steps: [
      ...
      "<b>[🖨️ 인쇄 / PDF 변환]</b> · <b>[🔑 답지만]</b>(정답만 인쇄) · <b>[💾 사이트 저장]</b>",
    ],
  ...
  exam: {
    title: "🧾 동형 모의고사 만드는 법",
    lead: "기출 시험지의 유형 구성을 본떠, 내 지문으로 같은 모양의 시험지를 만듭니다. 관리자 승인을 받은 선생님에게만 보이는 탭입니다.",
    steps: [
...
/* ── 만드는 법에 붙는 '이런 게 나옵니다' 예시 ──
   두 가지 방식이 섞여 있다.
   ① 상세분석·객관식·주관식·워크북 — **실제로 만든 결과물 PDF의 쪽 그림**을 보여 준다
      (public/howto-<탭>-N.jpg, samplePagesHtml). 짧게 지어낸 본보기보다 실물 한 쪽이
      "무엇이 나오는지"를 훨씬 잘 보여 준다는 판단이다(2026-09-26). 대가로, 결과물
      디자인을 바꾸면 **그림은 저절로 따라오지 않는다** — 인쇄물 모양이 눈에 띄게
      바뀌면 PDF를 새로 뽑아 그림을 갈아 끼울 것. 원본은 홍보/ 폴더(저장소 밖)의
      2026-09-20 홍보용 PDF 네 개다.
   ② 요약분석·단어장·동형 모의고사 — 결과물 PDF가 없어 **결과물을 실제로 그리는
      그 함수**(buildBriefHtml·buildQuizHtml)에 짧은 본보기 자료를 넣어 그 자리에서
      그린다. 그리는 코드나 CSS를 고치면 예시도 같이 바뀐다.

   어느 쪽이든 예시를 보려고 요금이 나가면 안 된다 — AI를 부르지 않는다. */
...
/* 요약 이미지 예시 — 그림은 AI가 지문마다 새로 그리는 것이라, 예시를 열 때마다
   ...
   (IMG_LANG_NAME의 값과 같은 말을 쓴다). 만드는 법에서는 이제 요약분석 탭만 이 함수를
   쓴다(sample-brief-*). 상세분석은 PDF 쪽 그림(samplePagesHtml)으로 바뀌었고,
   sample-analyze-*는 첫 화면과 소개 페이지(analyze.html)가 계속 쓴다 — 지우지 말 것.
...
function samplePagesHtml(tab, captions, note) {
  const shots = captions
    ...
    .join("");
  return `<div class="howto-pages">${shots}</div>
    <p class="howto-pages-note">${note || "실제로 만든 결과물의 일부입니다 — 지문에 따라 구성과 내용은 매번 달라집니다."}</p>`;
}

// 탭 → 예시 HTML. 없는 탭은 예시를 붙이지 않는다.
const HOWTO_SAMPLE = {
  analyze: () =>
    samplePagesHtml("analyze", [
      "문장 분석 — 1~3번 문장",
      "문장 분석 — 4~6번 문장 (출제 포인트 포함)",
      "Ⅱ. 주제 & 흐름 요약",
...
function openHowto(tab) {
  ...
  howtoSampleEl.querySelectorAll(".howto-page img").forEach((img) => {
    img.addEventListener("error", () => {
