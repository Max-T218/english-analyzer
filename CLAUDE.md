...
## 진행 중 — 문제 유형 보강 (실제 기출 대조)

동형모의고사(9개 학교, 71쪽)를 실제로 읽어 이 앱이 못 만드는 문제 유형을 찾아
하나씩 추가하는 중입니다. 분석 방법과 우선순위 근거는 커밋 메시지에 다 적어 뒀습니다.

**끝난 것** (커밋 순서대로): `c1fa363` 질문에 답하기(주관식) · `af452dc` 목적·심경·
지칭 추론(객관식) · `fd75d55` 어법 분석·옳은 문장 찾기·영영풀이 오류 찾기(객관식).

**안 끝난 것** — 빈도가 낮아 뒤로 미룬 세 가지, 아직 코드에 없습니다:
- **요약문 완전구성형** — 지금 "요약문 완성"은 AI가 만든 문장의 빈칸 2곳만 채우는데,
  실제 시험은 <보기> 단어은행으로 요약문 전체를 새로 짓게 하는 경우가 많았습니다.
  "조건 영작"과 비슷하지만 대상이 '지문 속 문장'이 아니라 '새로 짓는 요약문'이라 다릅니다.
- **연결어 2빈칸 추론** — "(A) Therefore (B) However"처럼 담화표지 두 자리를 고르는,
  수능에 흔한 유형. 지금 "빈칸"은 한 자리만 지원합니다.
- **다의어 문맥의미 매칭형** — 같은 단어(예: as)가 지문 여러 곳에서 다른 뜻으로 쓰일 때,
  같은 뜻끼리 <보기>에서 골라 기호를 쓰게 하는 유형. 1회 관찰, 드문 편입니다.

세 개 다 처리하고 나면 이 섹션은 지워도 됩니다.
