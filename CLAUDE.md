...
**`outline`은 반대입니다 — 이름이 같지만 일부러 다릅니다.** 상세분석
(`GEMINI_SCHEMA`)의 것은 종이 한 쪽을 쓰는 다섯 덩어리이고, 소책자(`BRIEF_SCHEMA`)의
것은 '한 줄 요약 + 흐름도' 둘뿐입니다. 소책자는 들고 다니라고 얇게 만드는 물건이라
두께를 늘릴 수 없어서입니다(예전 요약표 125mm ↔ 축소판 124mm로 맞췄고, 그래서 값도
200원 그대로입니다). **한쪽을 고칠 때 다른 쪽을 따라 고치지 마세요.** 프롬프트도
2026-09-04에 공유(`_prompt_part`)를 끊고 각자 쓰도록 갈라 두었습니다.
